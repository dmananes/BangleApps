...
var osm
try {
  // if it's installed, use the OpenStreetMap module
  osm = require('openstmap')
} catch (e) {}

function loadSettings() {
  settings = require('Storage').readJSON('apptivate.json', 1) || {}
  var changed = false
  if (!settings.file) {
    changed = true
  ...
  if (changed) require('Storage').writeJSON('apptivate.json', settings)
}
loadSettings()
...
function getTrackNumber(filename) {
  var trackNum = 0
  var matches = filename.match(/^apptivate\.log(.*)\.csv$/)
  if (matches) {
...
function showMainMenu() {
  ...
  const mainmenu = {
    ...
    /*LANG*/ RECORD: {
      ...
      onchange: (v) => {
        setTimeout(function () {
          E.showMenu()
          WIDGETS['apptivate'].setRecording(v).then(function () {
            print(/*LANG*/ 'Complete')
            loadSettings()
    ...
    /*LANG*/ 'File #': {
      ...
      onchange: (v) => {
        settings.recording = false // stop recording if we change anything
        settings.file = 'apptivate.log' + v + '.csv'
        settings.filefr = 'apptivate.logfr' + v + '.csv'
        updateSettings()
      }
    },
    /*LANG*/ 'View Tracks': () => {
  ...
  var recorders = WIDGETS['apptivate'].getRecorders()
...
function viewTracks() {
  ...
  var found = false
  require('Storage')
    .list(/^apptivate\.log.*\.csv$/, { sf: true })
    .forEach((filename) => {
      found = true
      menu[/*LANG*/ 'Track ' + getTrackNumber(filename)] = () => viewTrack(filename, false)
    })
  if (!found) menu[/*LANG*/ 'No Tracks found'] = function () {}
  menu['< Back'] = () => {
    showMainMenu()
  }
...
function viewTrack(filename, info) {
  ...
  if (info.time) menu[info.time.toISOString().substr(0, 16).replace('T', ' ')] = function () {}
  ...
  function plotTrack(info) {
    ...
    if (info.qOSTM) {
      ...
      getMapXY = function (lat, lon) {
        'ram'
        return { x: cx + Math.round((long - info.lon) * info.lfactor * info.scale), y: cy + Math.round((info.lat - lat) * info.scale) }
      }
    }
  ...
  function plotGraph(info, style) {
    ...
    if (style == 'Altitude') {
      ...
      // now iterate
      var p,
        lp = Bangle.project({ lat: c[1], lon: c[2] })
      var t,
        dx,
        dy,
        d,
